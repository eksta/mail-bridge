...
- **TB FCC dupe-check (critical)**: TB ≥102 searches Sent for a copy with matching Message-ID after each send; if not found it waits forever (no APPEND fallback), hanging the progressbar. The bridge remembers original client headers from SMTP DATA (`parse_outgoing` → `message_id`, `to_header`, etc.), maps them to the Yandex mid when the auto-filed copy appears (`_match_pending_send`), and `_restore_original_headers` rewrites the bridge-served copy so TB recognizes it. If headers break again, symptom = progressbar hangs + `Processing fcc` never appears in TB's `mailnews.send` MOZ_LOG.
